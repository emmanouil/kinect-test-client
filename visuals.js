...
function do_viz(projC){
	...
	projC.forEach(function(item, index, array){
		if(index == 7){ colour = 'rgba(255,0,0,1)'; system.addParticle(2*parseInt(item[0]), 2*parseInt(item[1]), index);
		}else if(index == 11){ colour = 'rgba(0,255,0,1)'; system.addParticle(2*parseInt(item[0]), 2*parseInt(item[1]), index);
		}else{ return; }
